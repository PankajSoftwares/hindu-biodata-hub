...
import { TEMPLATES, sampleBiodata, FORM_SECTIONS, type TemplateId } from "@/lib/biodata";
...
function CreatePage() {
  const { data, template, setTemplate, update, reset, savedAt, loadSample } = useBiodataDraft();
  ...
  const filled = useMemo(() => {
    ...
    const done = keys.filter((k) => String(data[k] ?? "").trim().length > 0).length;
      ...
      <div className="mt-6 grid gap-6 md:grid-cols-[1fr_1.05fr]">
        ...
        <div className={cn("md:block", mobileView === "form" ? "block" : "hidden")}>
          ...
          <div className="rounded-xl border border-border/70 bg-card/40 p-5">
            <BiodataForm data={data} update={update} />
            <div className="mt-6 flex items-center gap-2 text-xs text-muted-foreground">
              <Save className="h-3.5 w-3.5" /> Changes are saved automatically.
            </div>
          </div>
        </div>
          ...
          <div className="sticky top-20">
            <div className="overflow-x-auto rounded-xl border border-border/70 bg-[#eee5d3]/50 p-4">
              <div className="origin-top scale-[0.85] sm:scale-90 md:scale-100">
                <BiodataPreview id="biodata-print" data={data} template={template} />
              </div>
            </div>
          </div>
        </div>
